    ...
    let lessonUpdate = async (req) => {
        ...
        if (valError.length > 0) {
            ...
            if (sAction.toLowerCase() == 'add') {
                qry = `INSERT INTO lessons (name, description, imageFile, videoFile, videoDuration, categoryId, instructorId) 
                            VALUES ("${objLesson.lessonName}", "${objLesson.description}",  "${objLesson.imageFile}", 
                                    "${objLesson.videoFile}", "${objLesson.videoDuration}", ${objLesson.categoryId}, ${objLesson.instructorId} ) `;
                ...
                if (sMessageInfo == '') {
                    ...
                    if (objLesson.videoDuration != undefined) {
                        sSet = sSet + `videoDuration = "${objLesson.videoDuration}", `;
                    }
